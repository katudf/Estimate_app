...
from PySide6.QtWidgets import (
    QMessageBox, QComboBox, QCompleter, QLineEdit, # QComboBox, QCompleter, QLineEdit をインポート
...
# ↓↓↓ Slot, QEvent をインポートに追加 ↓↓↓
# ↓↓↓ QPalette, QColor, イベントクラス をインポート ↓↓↓
from PySide6.QtCore import Qt, Signal, Slot, QEvent, QModelIndex, QItemSelectionModel
from PySide6.QtGui import QPalette, QColor, QDropEvent, QDragEnterEvent, QDragMoveEvent, QContextMenuEvent, QAction, QKeySequence, QBrush
# ↑↑↑ # QContextMenuEvent, QAction をインポート
# ↑↑↑ QPalette, QColor をインポート ↑↑↑
# ↓↓↓ 定数とコマンド、ユーティリティをインポート ↓↓↓
from constants import WIDGET_BASE_STYLE, COLOR_LIGHT_GRAY, COLOR_WHITE, COLOR_ERROR_BG # <- COLOR_ERROR_BG を追加
# ↓↓↓ コマンドクラスをインポート ↓↓↓
from commands import AddRowCommand, InsertRowCommand, RemoveRowCommand, MoveRowCommand, ChangeItemCommand, DuplicateRowCommand # DuplicateRowCommand をインポート
# ↑↑↑ 定数をインポート ↑↑↑
# ↓↓↓ ユーティリティ関数をインポート ↓↓↓
...
class DraggableTableWidget(QTableWidget):
    ...
    def _configure_drag_drop(self):
        ...
        self.setSelectionBehavior(QAbstractItemView.SelectRows) # 行選択モード
...
# detail_page_widget.py の _on_cell_changed メソッドを修正
    ...
    @Slot()
    def remove_row(self):
        """現在選択されている行を削除する (UNDO対応)"""
        current_row = self.table.currentRow()
        if current_row >= 0: # 行が選択されている場合のみ
            # 削除する行のデータを取得 (クローンしておく)
            row_data_cloned = []
            for col in range(self.table.columnCount()):
                item = self.table.item(current_row, col)
                row_data_cloned.append(item.clone() if item else None)

            command = RemoveRowCommand(self.table, current_row, row_data_cloned)
            self.undo_stack.push(command)
            # テーブル操作と合計更新はコマンドの redo/undo と indexChanged で行われる
        else:
             # 選択されていない場合は何もしない (またはメッセージ表示)
             pass
